  ...
  methods: {
    ...
    list: function(path) {
      if (webview && webview.fileList) {
        var fc = this;
        webview.fileList(path, function(reason, files) {
          if (files) {
        ...
        this.error('webview.fileList is not available');
      }
    },
...
Vue.component('file-chooser-input', {
  ...
  methods: {
    nameChanged: function() {
      ...
      var dir = this.files.find(function(file) {
        return file.isDirectory && file.name === value;
      });
      if (dir) {
        this.list(this.path + '/' + dir.name);
      }
    },
    clean: function() {
      if (this.path === '') {
